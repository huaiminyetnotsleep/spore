...
func (s *Store) InsertDumpEntry(ctx context.Context, in DumpEntry) (DumpEntry, error) {
	...
	res, err := s.ex.ExecContext(ctx, `INSERT INTO dump_entries
		(channel_key, message_id, dump_ids_json, created_at) VALUES (?,?,?,?)`,
		in.ChannelKey, in.MessageID, string(idsJSON), in.CreatedAt)
	if err != nil {
...
// LatestDumpEntry 取同链接最新一条干净副本坐标；无匹配返回 ErrNotFound。
...
func (s *Store) LatestDumpEntry(ctx context.Context, channelKey string, messageID int) (DumpEntry, error) {
	...
	err := s.ex.QueryRowContext(ctx,
		`SELECT id, channel_key, message_id, dump_ids_json, created_at
		FROM dump_entries WHERE channel_key = ? AND message_id = ?
		ORDER BY id DESC LIMIT 1`, channelKey, messageID).
		Scan(&e.ID, &e.ChannelKey, &e.MessageID, &idsJSON, &e.CreatedAt)
