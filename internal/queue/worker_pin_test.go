...
func TestProcessPinTaskPartialResultText(t *testing.T) {
	...
	// 原消息来源卡片开头，置顶目标（带跳转链接、逐行）与失败目标随后
	cardPos := strings.Index(confirm, `<blockquote><b>🔗 原消息</b>`)
	pinPos := strings.Index(confirm, "已置顶到：")
	if confirm == "" || cardPos != 0 || pinPos < 0 ||
		!strings.Contains(confirm, `<a href="https://t.me/mychan">我的频道</a>`) ||
		!strings.Contains(confirm, "置顶失败：\n<blockquote>我的群组") {
		t.Fatalf("确认文案应以原消息来源卡片开头并含置顶明细，得到 %q", sender.texts())
	}
	if cardPos > pinPos {
		t.Fatalf("原消息卡片应在置顶结果之前: %q", confirm)
	}
}
