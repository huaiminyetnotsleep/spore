...
- 📨 **受保护消息提取**：文本（保留实体格式）、图片、视频、文件、音频、语音条与相册；超过 Bot API 50MB 上限的大文件经 Bot 身份 MTProto 直传（上限 2000MB），超过 2000MB 自动分卷拆分——切段后以同一条相册消息送达（上限约 19GB），附合并提示。
