...
- 📨 **受保护消息提取**：文本（保留实体格式）、图片、视频、文件、音频、语音条与相册；超过 Bot API 50MB 上限的大文件经 Bot 身份 MTProto 直传（上限 2000MB），超过 2000MB 自动分卷拆分——视频切成可直接播放的分段（ffmpeg 流复制、无损）与其他成员同一条相册送达（上限约 17.6GB），无需下载合并。
- 🤖 **Bot 交互**：私聊发链接即可使用；`/start` 申请、额度查询、进度、取消、频道绑定与加入等命令齐备，配额与重试边界明确。
- ☁️ **云盘下载**：`/download` 把提取的媒体直接上传到网盘（经 rclone），不再重发回 Telegram；支持多目的地、排队与取消；云盘任务走队列低优先级通道，仅在无转发任务排队时执行；`/pin` 输入引导可选「置顶+转存」一次提交两路。
- ♻️ **缓存频道复用**：成功投递后同步写无脚注干净副本到自有缓存频道，同链接再次提交整条复制秒回——不限媒体大小、相册保组。
- 🖥️ **Web 管理端**：内置 SPA 管理面板——用户审批与管理、请求记录、频道统计、运行设置、云盘配置、资源监控与审计。
- 🐞 **错误日志中心**：请求管线与 Bot 相关环节的报错逐条落库（来源/环节/错误码/原始根因串/参数快照），管理端按来源、错误码、请求、时间范围筛选查询，请求详情页可深链反查该请求全部环节错误；默认保留 30 天自动清理，支持手动批量与按时间段清理。
...
- 👁 **监听源预热**：`/watch` 把你管理的频道/超级群组配置为监听源（bot 需为该聊天管理员），源内新消息自动转存缓存频道——之后任何人提交该源的消息链接都直接命中复用秒回；用户自助申请可配审批/免审批与数量上限，管理端也可直接添加与审批。
- 💾 **定时备份与 R2 上云**：默认每 6 小时生成数据库一致性快照，本地保留 8 份（48 小时滚动窗口）；可选直传 Cloudflare R2 异地保存（全量 ZIP、远端同份数轮转、凭据不出机器），配置与恢复见 [R2 备份配置指南](docs/guide/r2-backup.md)。
...
运行后出现管理菜单，选 **1** 安装：选择镜像版本（留空回车 = latest；填 `1.16.4` 等可固定/回滚到指定版本）→ 自动下载配置到 `~/spore` → 交互式填写 `BOT_TOKEN` / `TG_API_ID` / `TG_API_HASH`（可留空回车跳过，之后在 `.env` 补填）→ 选择宿主端口（默认 8080，自动检测占用）→ 授权 `data/` → 自选是否立即启动（不启动则稍后 `spore restart` 首启）→ 启动时打印首次访问密钥。安装完成后会把脚本注册为 **`spore` 命令**，之后直接输入 `spore` 打开管理菜单，或 `spore status` / `spore upgrade` 等子命令完成**升级、验证、状态、日志、查看/重设密钥、清理临时文件、磁盘检查、重启、停止、卸载**等运维操作；部署目录可经 `curl ... | SPORE_DIR=/opt/spore bash` 覆盖。详见[部署指南](docs/guide/deployment.md)。

```bash
spore                # 打开管理菜单（安装/升级/验证/状态/日志/密钥/清理/重启/停止/卸载）
spore status         # 例：查看运行状态（免菜单直通）
spore upgrade        # 例：升级到最新镜像
spore install 1.16.4 # 例：安装/切换/回滚到指定版本（spore install latest 解除固定）
```
