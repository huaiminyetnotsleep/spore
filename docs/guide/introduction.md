...
- **统一「下载 → 上传」，按大小路由**：≤50MB 走 Bot API 上传，超限走 Bot 号 MTProto 直传（上限 2000MB），超限视频自动切为可播放分段整组送达。下载与上传重叠（边下边发），按媒体大小分流式 / 内存 / 临时文件三档策略；内存路径受进程级预算闸门约束（预算不足自动降级临时文件路径），内存占用被固定额度封顶、不随并发任务数放大。
