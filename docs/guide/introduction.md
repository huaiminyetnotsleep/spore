...
- **受保护消息提取**：支持文本、图片、视频、文件、音频、语音条与相册，保留加粗、链接等实体格式；超过 Bot API 50MB 上限的大文件经 Bot 身份 MTProto 直传（上限 2000MB），再超过的自动分卷拆分——切段后以同一条相册消息送达（上限约 19GB），附合并提示。
...
- **统一「下载 → 上传」，按大小路由**：≤50MB 走 Bot API 上传，超限走 Bot 号 MTProto 直传（上限 2000MB），超限媒体自动分卷拆分整组送达。下载与上传重叠（边下边发），按媒体大小分流式 / 内存 / 临时文件三档策略；内存路径受进程级预算闸门约束（预算不足自动降级临时文件路径），内存占用被固定额度封顶、不随并发任务数放大。
