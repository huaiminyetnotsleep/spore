...
异常事件会进入 Web 事件中心。管理端“通知设置”可在同一页面配置通知渠道、事件规则和静音计划；
开启系统事件自动通知后，事件按策略投递到已启用的 Bot/Webhook。未开启配置化通道时，
兼容的 owner Bot 私聊仍按冷却窗口工作；屏蔽或静音只停止提醒，不删除事件记录。
活动通知（管理后台登录成功、新用户申请、频道加入申请）逐次即时推送关键信息，
默认开启，不进入 Web 事件中心（登录等行为以审计日志留痕），可在通知策略中按事件或类别关闭。
...
除手动导出外，服务内置**定时备份**（默认每 6 小时、本地 `data/backups/`
保留最近 8 份 ≈ 48 小时滚动窗口；间隔与份数在管理端「备份 → 定时备份与
云端同步」调整，0 为关闭）。定时备份失败（含磁盘空间不足跳过）会进入
事件中心告警，CLI `spore admin backup` 走同一逻辑。

可选开启 **Cloudflare R2 异地直传**：本地快照成功后，同一份快照会打成
全量 ZIP（数据库 + JSON 配置，排除 `r2-backup.json` 与 `cloud-drive.json`
两个凭据文件）直传 R2 并按同份数轮转。R2 凭据只存 `data/r2-backup.json`
（0600），不进数据库、不进任何备份件。配置步骤与恢复方法见
[R2 备份配置指南](../guide/r2-backup.md)。上云失败只告警不影响本地备份；
`last_backup_at` 按本地快照口径记录，下一轮定时备份会用新快照重试上传。

从 R2 恢复：Cloudflare 控制台下载 `spore-full-backup-YYYYMMDD-HHMMSS.zip`
→ 本地解压 → `.db` 走管理端整库恢复（下次启动应用）、JSON 文件走
配置文件恢复。机器全损时同一路径适用。
...
状态页会给出离线原因分类（封禁 / 会话撤销 / 网络）与对应处置指引；**账号被封禁或
会话被撤销**的完整处置流程（导出备份 → 清理会话文件 → 新号扫码）与各封禁场景的
应急手册见 [incidents.md](./incidents.md)。
...
任何「请求失败 / Bot 收发异常 / 网盘与备份失败 / 监听转储失败 / 用户号会话离线」的具体
原因，优先在管理端「事件与审计 → 错误日志」页查询，不必进容器翻 docker 日志：

- **筛选**：来源（请求管线 / Bot 收发 / 网盘 / 备份 / 监听源 / 用户号会话）、错误码、
  级别（错误 / 警告）、请求 ID、时间范围；行展开可见**原始根因串**（如 `FLOOD_WAIT_9:
  3000`、rclone 的 `couldn't login` 报错）与参数快照（任务 / Bot / 频道 / 目的地）。
- **按请求反查**：请求详情页错误区块的「查看相关日志」深链直达该请求全部环节的逐次
  留痕——重试期间的中间失败根因不再被终态覆盖丢失。
- **与事件中心的关系**：事件中心按 key 聚合（管要不要通知），错误日志逐条留痕（管
  到底发生了什么）；收到告警后来这里查根因。
- **清理**：默认保留 30 天自动清理（运行设置 → 错误日志保留天数，1–365 可调、即时
  生效）；错误日志页另有手动批量删除与「按时间段清理」（先显示将删除条数再确认）。
