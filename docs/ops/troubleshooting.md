...
## 2026-09-21 监听源列表报 `no such column: w.kind` → 恢复 v17 原迁移并追加 v18

**现象（真机）**：打开管理端监听源列表时，服务日志持续报：

```text
STORE_UNAVAILABLE: 列出监听源: SQL logic error: no such column: w.kind
```

**根因**：开发期间 v17 迁移已被某个中间构建实际部署并把数据库
`user_version` 推到 17；随后代码原地修改了 v17 脚本、给 `watch_sources`
增加 `kind` / `bot_id` / `bot_username` 和 `watch_events`。已到 v17 的真实库不会
重跑 v17，因此代码查询新列、数据库仍是首版结构。

**方案**：
- v17 恢复为首次部署时的原始结构，保持已发布迁移不可变；
- 追加 v18：新建最终形态 `watch_sources_v18`，仅从旧结构共有列复制既有行，
  新列取默认值，再替换原表；以 `IF NOT EXISTS` 创建 `watch_events` 与索引；
- 增加真实升级回归测试：依次执行 v1–v17、写入既有监听源、设置
  `user_version=17`，再由当前 `Store.Open` 升到 v18，断言旧行保留、新列默认、
  `watch_events` 可读写、列表联表不再报错、重复打开幂等。

**要点（防复发）**：
- “还没正式 release”不等于迁移未发布；只要任何真机/测试环境启动过，就必须
  视为已发布。迁移脚本永远只增不改；开发中的追加列也走新版本。
- SQLite `PRAGMA user_version` 不接受绑定参数；测试/迁移必须用受控整数格式化。
- 多种中间结构需要收敛时，重建表时只引用所有旧结构共有列，新增列靠 DEFAULT，
  避免 `ALTER TABLE ADD COLUMN` 在某些中间库已存在该列时失败。
