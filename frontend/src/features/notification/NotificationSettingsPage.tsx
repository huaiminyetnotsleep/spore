...
function credentialTag(status: "missing" | "configured" | "unavailable") {
  if (status === "configured") return <Tag color="green">已配置</Tag>;
  if (status === "unavailable") return <Tag color="red">凭据不可用</Tag>;
  return <Tag>未配置</Tag>;
}
...
export function NotificationSettingsPage() {
  ...
  const save = useAdminAction({
    ...
    invalidate: [["notification", "config"]],
  ...
  return (
    <PageScaffold
      title="通知设置"
      description="配置 Telegram Bot 与 Webhook 通知通道。敏感字段永不回显，留空保存表示沿用已保存值。"
    >
      <PageQueryState
              ...
              },
              {
                ...
                children: (
                  <PageSection
                    ...
                    extra={data ? credentialTag(webhookURLStatus) : null}
                  >
                    <Space direction="vertical" size="middle" className="field-width-full">
