 ...
 * （静音/归档）、外部频道治理（自动退出外部拉入）；开关类切换即保存、
 * 即时生效（joinmgr 每次实时读取 syscfg），保存中的开关展示 pending；
 * 加入上限为数字输入，需显式点「保存上限」（按钮紧邻控件并展示 pending）。
 */
...
export function JoinSettingsPage() {
  ...
  return (
    <PageScaffold
      title="受邀设置"
      description="Bot /join 邀请加入的功能开关、加入策略、加入后处理与外部频道治理；开关类配置切换即保存，加入数量上限需显式保存。"
    >
      <PageQueryState
      ...
      >
        <Space direction="vertical" size="middle" className="field-width-full">
          <Alert
            type="info"
            showIcon
            message="开关类配置切换即保存并即时生效，无需重启；加入数量上限修改后需点击「保存上限」提交。"
          />

          <PageSection title="功能开关" extra={<Tag color="green">切换即保存</Tag>}>
            <SettingItem
              control={
                <Switch
          ...
          <PageSection title="加入后处理" extra={<Tag color="green">切换即保存</Tag>}>
            <Space direction="vertical" size="middle" className="field-width-full">
              <SettingItem
                control={
                  <Switch
          ...
          <PageSection title="外部频道治理" extra={<Tag color="green">切换即保存</Tag>}>
            <Space direction="vertical" size="middle" className="field-width-full">
              <SettingItem
                control={
                  <Switch
