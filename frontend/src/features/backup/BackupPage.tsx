 ...
 * 确认弹窗文案与 SSR data-confirm 对齐；失败只展示服务端受控文案，
 * 不误报成功。备份内容不落 localStorage。
 */
import { useQuery } from "@tanstack/react-query";
import { DownloadOutlined, UploadOutlined } from "@ant-design/icons";
import { Alert, Button, Space, Table, Typography, Upload } from "antd";
...
export function BackupPage() {
  ...
  const confirm = useAdminAction({
    action: () => confirmBackupImport(),
    invalidate: [["backup"]],
    successText: (result) => result.message,
  });

  // 破坏性/耗时操作统一走确认弹层（文案与 SSR data-confirm 一致）
    ...
    <PageCard title="数据备份">
      <Space direction="vertical" size="middle" className="field-width-full">
        ...
        <Button
          ...
          onClick={() => confirmAction(CONFIRM_EXPORT_TEXT, () => void exportAction.run(undefined))}
        >
          导出数据库备份
        </Button>

        <SectionCard title="导入业务数据库">
          <Paragraph type="secondary" className="layout-margin-top-0">
              ...
              action={
                data.pending_state === "待确认" ? (
                  <Button
                    ...
                    onClick={() => confirmAction(CONFIRM_IMPORT_TEXT, () => void confirm.run(undefined))}
                  >
                    确认导入（下次启动应用）
                  </Button>
                ) : undefined
